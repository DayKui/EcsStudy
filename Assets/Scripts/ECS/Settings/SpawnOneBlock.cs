    ...
    public class SpawnOneBlock : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            if (Prefab_ref)
            {
                ...
                manager.Instantiate(Prefab_ref, entityArray);//把预制体转为Entity

                manager.SetComponentData(entityArray[0], new Position { Value = new float3(4, 0f, 0f) });
                manager.SetComponentData(entityArray[1], new Position { Value = new float3(6, 0f, 0f) });
                manager.SetComponentData(entityArray[2], new Position { Value = new float3(-2, 0f, 0f) });
